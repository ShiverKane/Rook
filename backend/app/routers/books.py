...
def list_books(db: Session = Depends(get_db)):
    return db.query(Book).filter(Book.is_approved == True).order_by(Book.id.desc()).all()
...
@router.get("/admin/pending", response_model=List[BookOut])
def admin_pending_books(db: Session = Depends(get_db), current_user: User = Depends(get_current_user)):
    if current_user.role != 'admin':
        raise HTTPException(status_code=status.HTTP_403_FORBIDDEN, detail="Admin access required")
    return db.query(Book).filter(Book.is_approved == False).order_by(Book.id.desc()).all()

@router.patch("/admin/{book_id}/approve", response_model=BookOut)
def admin_approve_book(book_id: int, db: Session = Depends(get_db), current_user: User = Depends(get_current_user)):
    if current_user.role != 'admin':
        raise HTTPException(status_code=status.HTTP_403_FORBIDDEN, detail="Admin access required")
    book = db.query(Book).filter(Book.id == book_id).first()
    if not book:
